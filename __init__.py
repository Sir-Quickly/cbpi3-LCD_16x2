...
def init(cbpi):
    ...
    try:
        lcd = lcd(LCDaddress)
        lcd.create_char(0, bierkrug0)
        lcd.create_char(2, bierkrug1)
        lcd.create_char(3, bierkrug2)
        lcd.create_char(4, bierkrug3)
        lcd.create_char(5, bierkrug4)
        lcd.create_char(6, wheel)
...
class screen:
  ...
  def get_item():
    return self.item_to_display

  def set_item(item):
    self.active_item = item
  
  def get_property():
    return self.property_to_display

  def set_properety(prop):
    self.active_property = prop

  def next_item():
    self.active_item %= max_items
    self.active_item += 1
  
  def next_property():
    active_property += 1
    if active_property > max_properties:
      self.set_property (1)
      self.next_item()
  
  pass #end of claSS screen

def lcd(LCDaddress):
    try:
        lcd = CharLCD(i2c_expander='PCF8574', address=LCDaddress, port=1, cols=16, rows=2, dotsize=8, charmap='A00', auto_linebreaks=True, backlight_enabled=True)
        return lcd
    except:
        pass
...
def show_multidisplay(refresh):
    ...
    cbpi.app.logger.info("Kettle Dictionary %s" % (cbpi.cache.get("kettle")))
    for id1, value in cbpi.cache["kettle"].iteritems():
      ...
      stirrer_of_kettle = int(cbpi.cache.get("kettle").get(id1).agitator)
      ...
      stirrer_status = cbpi.cache.get("actors").get(stirrer_of_kettle).state
    
      #line1 the stepname
      #line1 = (u'%s' % (s.name,)).ljust(20)[:20]
      line1 = (u"%s" % (cbpi.cache.get("kettle")[id1].name)).ljust(16)[:16]
      global line2_state
      if line2_state == 0:
	  ...
	  line2_state = 3
		...
		line2 = (u"Time: %s" % (time_remaining).ljust(16))[:16]
	    else:
		line2 = (u"Time: %s" %("stopped").ljust(16))[:16]
...
def show_standby(ipdet):
    lcd.cursor_pos = (0, 0)
    lcd.write_string((u"CraftPi %s" % cbpi_version).ljust(16))
    lcd.cursor_pos = (1, 0)
    lcd.write_string((u"%s" % (cbpi.get_config_parameter("brewery_name","No Brewery"))).ljust(16)[:16])
#    lcd.cursor_pos =(2, 0)
...
@cbpi.backgroundtask(key="lcdjob", interval=2)
def lcdjob(api):
    ## YOUR CODE GOES HERE    
    ## This is the main job
    cbpi.app.logger.info("LCDDisplay  - Roland was here")
	...
	cbpi.app.logger.info("LCDDisplay  - Displaying Multi-Display")
        show_multidisplay(refreshTime)
        
    elif s is not None and multidisplay_status == "off":       
	cbpi.app.logger.info("LCDDisplay  - Displaying Single-Display")
        show_singlemode(int(set_parameter_id1()))

    elif is_fermenter_step_running() == "active":
	cbpi.app.logger.info("LCDDisplay  - Displaying Fermentation Display")
        show_fermentation_multidisplay(refreshTime)

    else:
	cbpi.app.logger.info("LCDDisplay  - Displaying Standy Screen")
        show_standby(ip)
    pass
